...
int suse_create(int pid, int tid) {
	printf("Pido un create para el tid %d \n", tid);
	int operacion = CREATE;
	void* mensaje = malloc(8);
	memcpy(mensaje,&operacion,4);
	memcpy(mensaje+4,&tid,4);
	enviarPorSocket(socket_suse,mensaje,8);
	int resultado;
...
void hilolay_init(){
	...
	leer_config("/home/utnso/workspace/SUSE/hilolay/hilolay_config.txt");
